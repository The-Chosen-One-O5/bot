...
class KeepAliveServer:
    """Simple Flask server to keep Render service awake"""
    def __init__(self):
...
class TelegramSchedulerBot:
    ...
    async def help_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        help_text = (
            ...
            "• <code>/setrepeating HH:MM YYYY-MM-DD message</code> - Set recurring message\n"
            "  Example: <code>/setrepeating 04:00 2025-01-01 Happy New Year!</code>\n\n"
    ...
    async def status_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        for i, schedule in enumerate(schedules, 1):
            ...
            if msg_type == 'daily':
                ...
                if len(schedule['message_template']) > 50:
                    message_preview += "..."
                repeat_until = schedule['repeat_until_date']
                status_text += f"<b>{i}.</b> Repeating Message (ID: {schedule['id']})\n"
                status_text += f"   ⏰ {time}\n"
                status_text += f"   🔁 Until: {repeat_until}\n"
                status_text += f"   💬 {message_preview}\n\n"
    ...
    async def set_repeating_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if self.scheduler.add_repeating_message(chat.id, time_str, date_str, message):
            ...
            await update.message.reply_text(
                ...
                f"⏰ Time: {time_str} ({timezone})\n"
                f"📅 Repeats until: {date_str}\n"
                f"💬 Message: {message}\n\n"
                f"<i>This message will be sent daily at the specified time until the target date.</i>",
                parse_mode='HTML'
            )
        else:
...
def main():
    """Main function"""
    try:
        # Start keep-alive server in a separate thread for Render
        if os.environ.get('PORT'):  # Only start if PORT is set (Render environment)
            keep_alive = KeepAliveServer()
            server_thread = threading.Thread(target=keep_alive.run, daemon=True)
            server_thread.start()
            logger.info("Keep-alive server started for Render deployment")
