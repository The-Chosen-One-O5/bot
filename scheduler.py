    ...
    def _should_send_message(self, msg_config: Dict, current_time: datetime) -> bool:
        ...
        try:
            ...
            if (local_time.hour == schedule_hour and 
                ...
                # Check if repeating message has expired
                if msg_config["message_type"] == "repeating":
                    repeat_until_date_str = msg_config.get("repeat_until_date")
                    if repeat_until_date_str:
                        repeat_until_date = datetime.strptime(repeat_until_date_str, 
'%Y-%m-%d').date()
                        if local_time.date() > repeat_until_date:
                            # Message has expired, mark as inactive
                            self.db.remove_scheduled_message(msg_config["id"], msg_config["chat_id"])
                            return False
                return True
    ...
    def add_repeating_message(self, chat_id: int, time_str: str, until_date_str: str, message: str) -> bool:
        """Add a repeating scheduled message"""
        try:
            # Validate time format
            datetime.strptime(time_str, 
'%H:%M')
            # Validate until_date format
            datetime.strptime(until_date_str, 
'%Y-%m-%d')
            ...
            return self.db.add_scheduled_message(
                ...
                schedule_time=time_str,
                message_template=message,
                repeat_until_date=until_date_str
            )
        except ValueError as e:
